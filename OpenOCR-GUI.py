...
class OpenOCRGUI(QMainWindow):
    ...
    def merge_output_files(self, target_dir, base_name):
        # 合并 MD
        if self.chk_save_md.isChecked():
            md_files = natsorted([f for f in os.listdir(target_dir) if f.endswith('.md') and f != f"{base_name}.md"])
            if md_files:
        ...
        # 合并 JSON
        if self.chk_save_json.isChecked():
            json_files = natsorted([f for f in os.listdir(target_dir) if f.endswith('.json') and f != f"{base_name}.json"])
            if json_files:
